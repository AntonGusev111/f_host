...
class FileViewSet(ModelViewSet):
    ...
    def partial_update(self, request, pk=None):
        try:
            ...
            serialize_data['original_file_name'] = serialize_data['original_file_name'].split('.')[0]
            if self.request.user.is_staff or user_rights(request.user, pk):
                serializer = File_serializer([serialize_data, pk], data=serialize_data, partial=True)
                serializer.is_valid()
                serializer.save()
